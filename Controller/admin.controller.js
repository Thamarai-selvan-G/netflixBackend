...
let createAdmin = async (req, res) => {
  try {
    ...
    let checkUserEmail = await adminController.findAll({where: { email: inputEmail }});
    ...
    const genToken = await jwt.sign({ id: data.id }, secret, {
      expiresIn: "2hr",
    });
    ...
    return res.status(201).json({ message: "admin created succesfully...", result: data });
  } catch (error) {
