...
let createVendor = async (req, res) => {
  try {
    ...
    await vendorController
      .findAll({ where: { email: inputEmail } })
      .then(async (val) => {
        if (val.length == 0) {
          var salt = bcrypt.genSaltSync(10);
          var hash = await bcrypt.hash(req.body.password, salt);
          data["password"] = hash;
          await vendorController.create(data).then(async (value) => {
...
const loginId = async (req, res) => {
  try {
    ...
    const user = await vendorController.findOne({
    ...
    let updateToken = await vendorController.update(
      { token: newToken },
      { where: { id: user.id } }
    );
    if (updateToken) {
...
const delectAcc = async (req, res) => {
  try {
    ...
    let deleteUser = await vendorController.destroy({
      where: { email: inputEmail },
    });
    if (deleteUser) {
      return res.json({
